    ...
    methods: {
        saveBranch() {
            var data = {
                ...
                other_infomation: this.other_infomation,
            }
            common.loading.show('body');
            $.post('/config/save-branch', data)
                .done(response => {
                    if (response.error === 0) {
                        common.loading.hide('body');
                        bootbox.alert("Lưu thành công !!", function() {
                            window.location = '/config/branch';
                        })
                    } else {
                        bootbox.alert('Error!!!');
                    }
        ...
        deleteBranch(id_branch) {
            ...
            bootbox.confirm({
                ...
                callback: (result) => {
                    if (result) {
                        common.loading.show('body');
                        $.post('/config/delete-branch', data)
                        .done(response => {
                            if (response.error === 0) {
                                common.loading.hide('body');
        ...
        getInfoBranch(name, phone, address, other_info, latitude, longitude, id) {
            this.name_branch_update = name;
            this.phone_branch_update = phone;
            this.address_update = address;
            this.other_infomation_update = other_info;
        ...
        updateBranch() {
            var data = {
                ...
                other_infomation_update: this.other_infomation_update,
                id_branch_update: this.id_branch_update,
            }
            common.loading.show('body');
            $.post('/config/update-branch', data)
                .done(response => {
                    if (response.error === 0) {
                        common.loading.hide('body');
                        bootbox.alert("Lưu thành công !!", function() {
