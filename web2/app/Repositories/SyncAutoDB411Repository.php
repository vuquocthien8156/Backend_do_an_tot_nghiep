...
class SyncAutoDB411Repository {
    ...
    public function checkExist_SoCT($value) {
        try {
            ...
            $timestamp = Carbon::now();
            $content_error = "{$timestamp->format('Y:m:d H:i:s')}: Failed get orders id, code: {$value}. message: {$e->getMessage()}";
            Storage::append("error_sync_auto/error_{$timestamp->format('Y_m_d')}.txt", $content_error); //Error
        }
    }
    ...
    public function checkExist_MaXe($value) {
        try {
            ...
            $timestamp = Carbon::now();
            $content_error = "{$timestamp->format('Y:m:d H:i:s')}: Failed get membership_card. MaXe: {$value}. message: {$e->getMessage()}";
            Storage::append("error_sync_auto/error_{$timestamp->format('Y_m_d')}.txt", $content_error); //Error
        }
    }
    ...
    public function checkExist_MaLoai($value) {
        try {
            ...
            $timestamp = Carbon::now();
            $content_error = "{$timestamp->format('Y:m:d H:i:s')}: Failed get Model Category. MaLoai: {$value}. message: {$e->getMessage()}";
            Storage::append("error_sync_auto/error_{$timestamp->format('Y_m_d')}.txt", $content_error); //Error
        }
    }
    ...
    public function checkExist_MaHangXe($value) {
        try {
            ...
            $timestamp = Carbon::now();
            $content_error = "{$timestamp->format('Y:m:d H:i:s')}: Failed get Manufacture Category. MaHangXe: {$value}. message: {$e->getMessage()}";
            Storage::append("error_sync_auto/error_{$timestamp->format('Y_m_d')}.txt", $content_error); //Error
        }
    }
    ...
    public function checkExistGroupStaff_ID($value) {
        try {
            ...
            $timestamp = Carbon::now();
            $content_error = "{$timestamp->format('Y:m:d H:i:s')}: Failed get Group Staff Category. ID: {$value}. message: {$e->getMessage()}";
            Storage::append("error_sync_auto/error_{$timestamp->format('Y_m_d')}.txt", $content_error); //Error
        }
    }
}