...
class SyncAutoDB411Service {
    ...
    public function syncBranchAuto($id_sync_branch, $name, $address, $phone, $myJSON, $insert, $update, $delete) {
        ...
        try {
            ...
            if (isset($saveBranchSync) || isset($updateBranchSync) || isset($deleteBranchSync)) {
                ...
                Storage::append("error_sync_auto/error_branch_{$timestamp->format('Y_m_d')}.txt", $content_success); //Error
            }
        } catch (Exception $e) {
    ...
    public function syncManufactureAuto($id_manufacture_sync, $name, $type, $parent_category_id, $myJSON, $insert, $update, $delete) {
        ...
        try {
            ...
            if (isset($saveUserStaffSync) || isset($updateUserSync) || isset($deleteUserSync)) {
                ...
                Storage::append("error_sync_auto/error_membership_card_{$timestamp->format('Y_m_d')}.txt", $content_success); //Error
            }
        } catch (Exception $e) {
    ...
    public function syncMemberShipCardAuto($id_sync_membership_card, $id_user, $status, $name, $vehicle_number, $code, $id_manufacture, $id_model, $color, $created_at, $approved_at, $expired_at, $approved, $vehicle_card_status, $myJSON, $insert, $update, $delete) {
        ...
        try {
            ...
            if (isset($saveUserStaffSync) || isset($updateUserSync) || isset($deleteUserSync)) {
                $content_success = "{$timestamp->format('Y:m:d H:i:s')}: => Success! sync membership_card id_sync_membership_card: {$id_sync_membership_card}. Status: insert: {$insert}, update: {$update}, delete: {$delete}" ;
                Storage::append("error_sync_auto/error_membership_card_{$timestamp->format('Y_m_d')}.txt", $content_success); //Error
            }
        } catch (Exception $e) {
            $content_error = "{$timestamp->format('Y:m:d H:i:s')}:  Error! Failed sync membership_card id_sync_membership_card: {$id_sync_membership_card}. Status: insert: {$insert}, update: {$update}, delete: {$delete} . Error:  {$e->getMessage()}";
            Storage::append("error_sync_auto/error_membership_card_{$timestamp->format('Y_m_d')}.txt", $content_error); //Error
    ...
    public function getIdStaffByDataSync($value) {
        return $this->syncAutoDB411Repository->getIdStaffByDataSync($value);
    }

    public function getVehicleNumber($id_user) {
        return $this->syncAutoDB411Repository->getVehicleNumber($id_user);
    }

    public function getDetailRepairVehicle($value) {
        return $this->syncAutoDB411Repository->getDetailRepairVehicle($value);
    }
    ...
    public function syncOrderAuto($id_user, $vehicle_number, $id_sync_order, $code, $price, $name, $created_at, $completed_at, $id_staff, $number_km, $branch_id, $myJSON, $insert, $update, $delete) {
        ...
        try {
            if ($insert == true) {
                DB::beginTransaction();
                $saveOrderSync = $this->syncAutoDB411Repository->saveOrderAuto($id_user, $vehicle_number, $id_sync_order, $code, $price, $created_at, $completed_at, $id_staff, $number_km, $branch_id, $myJSON);
                $order_id = $saveOrderSync->id;
                $saveOrderDetail = $this->syncAutoDB411Repository->saveOrderDetailAuto($order_id, $name, $created_at, $type = EOrderType::ARBITRARY_SERVICE_ORDER, $price = null, $quantity = null, $myJSON);
                if (isset($saveOrderSync) && isset($saveOrderDetail)) {
                    DB::commit();
                } else {
                    DB::rollBack();
                }
            ...
            if (isset($saveOrderSync) || isset($updateOrderSync) || isset($deleteOrderSync)) {
                ...
                if (isset($saveOrderSync)) {
                    return $saveOrderSync;
                }
            }
        } catch (Exception $e) {
    ...
    public function saveOrderDetailRepairVehicle($order_id, $name, $created_at, $type, $price, $quantity, $myJSON) {
        $timestamp = Carbon::now();
        try {
            $saveOrderDetail = $this->syncAutoDB411Repository->saveOrderDetailAuto($order_id, $name, $created_at, $type, $price, $quantity, $myJSON);
    ...
    public function checkExist_SoCT($value) {
        return $this->syncAutoDB411Repository->checkExist_SoCT($value);
    }

    public function checkExist_MaXe($value) {
        return $this->syncAutoDB411Repository->checkExist_MaXe($value);
    }

    public function checkExist_MaLoai($value) {
        return $this->syncAutoDB411Repository->checkExist_MaLoai($value);
    }

    public function checkExist_MaHangXe($value) {
        return $this->syncAutoDB411Repository->checkExist_MaHangXe($value);
    }

    public function checkExistGroupStaff_ID($value) {
        return $this->syncAutoDB411Repository->checkExistGroupStaff_ID($value);
    }
}