...
class SyncDB411Auto extends Command {
    ...
    public function Sync_Auto_GroupStaff() {
        ...
        if (count($syncGroupStaffAuto) > 0) {
            foreach ($syncGroupStaffAuto as $key => $value) {
                ...
                $myJSON = json_encode($myObj3, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
                if ($value->SYS_CHANGE_OPERATION == "I") {
                    $checkExistGroupStaff_ID = $this->syncAutoDB411Service->checkExistGroupStaff_ID($value->CT_ID);
                    if(isset($checkExistGroupStaff_ID[0]->id)) {
            ...
            $timestamp = Carbon::now();
            $content = "{$timestamp->format('Y:m:d H:i:s')}: Group Staff: All data synced already!!";
    ...
    public function Sync_Auto_User_1() { // Table: dbo.KhachHangNgoai
        ...
        if (count($syncUserAuto1) > 0) {
            ...
            $timestamp = Carbon::now();
            $content = "{$timestamp->format('Y:m:d H:i:s')}: User (dbo.KhachHangNgoai): All data synced already!!";
    ...
    public function Sync_Auto_User_2() { // Table: dbo.KhachHangNgoai
        ...
        if (count($syncUserAuto2) > 0) {
            foreach ($syncUserAuto2 as $key => $value) {
                ...
                if ($value->SYS_CHANGE_OPERATION == "I") {
                    if ($value->DTDiDong != null && preg_replace('/\D/', '', $value->DTDiDong) != "") {
                        $check_phone_exist = $this->syncAutoDB411Service->checkPhoneExist(preg_replace('/\D/', '', $value->DTDiDong), $type = EUser::TYPE_USER);
                        if (isset($check_phone_exist[0]->id)) {
                            $id_user = $check_phone_exist[0]->id;
                            $meta_old = $check_phone_exist[0]->meta;
                            if ($meta_old == null) {
                            ...
                            $this->syncAutoDB411Service->syncUserAuto($value->CT_MaKH, $value->HoTen, preg_replace('/\D/', '', $value->DTDiDong), $value->EmailKH, $value->DiaChi, $myJSON, $insert = true, $update = false, $delete = false);
                        }
                    } else {
                        $this->syncAutoDB411Service->syncUserAuto($value->CT_MaKH, $value->HoTen, preg_replace('/\D/', '', $value->DTDiDong), $value->EmailKH, $value->DiaChi, $myJSON, $insert = true, $update = false, $delete = false);
                    }
                    ...
                    $this->syncAutoDB411Service->syncUserAuto($value->CT_MaKH, $value->HoTen, preg_replace('/\D/', '', $value->DTDiDong), $value->EmailKH, $value->DiaChi, $myJSON, $insert = false, $update = true, $delete = false);
            ...
            $timestamp = Carbon::now();
            $content = "{$timestamp->format('Y:m:d H:i:s')}: User(dbo.KhachHang): All data synced already!!";
            Storage::append("error_sync_auto/error_user_{$timestamp->format('Y_m_d')}.txt", $content); //Error
        }
    }
    // Membershipcard
    ...
    public function Sync_Manufacture_Auto() {
        ...
        if (count($syncManufactureAuto) > 0) {
            foreach ($syncManufactureAuto as $key => $value) {
                ...
                $myJSON = json_encode($myObj3, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
                if ($value->SYS_CHANGE_OPERATION == "I") {
                    $checkExist_MaHangXe = $this->syncAutoDB411Service->checkExist_MaHangXe($value->MaHangXe);
                    if(isset($checkExist_MaHangXe[0]->id)) {
    ...
    public function Sync_Model_Auto() {
        ...
        if (count($syncModelAuto) > 0) {
            foreach ($syncModelAuto as $key => $value) {
                ...
                if ($value->SYS_CHANGE_OPERATION == "I") {
                    $checkExist_MaLoai =  $this->syncAutoDB411Service->checkExist_MaLoai($value->CT_MaLoai);
                    if(isset($checkExist_MaLoai[0]->id)) {
            ...
            $timestamp = Carbon::now();
            $content = "{$timestamp->format('Y:m:d H:i:s')}: Model: All data synced already!!";
    ...
    public function Sync_MembershipCard_Auto() {
        ...
        if (count($syncMembershipCardAuto) > 0) {
            foreach ($syncMembershipCardAuto as $key => $value) {
                ...
                if ($value->SYS_CHANGE_OPERATION == "I") {
                    $checkExist_MaXe = $this->syncAutoDB411Service->checkExist_MaXe($value->CT_MaXe);
                    if(isset($check_Exist_MaXe[0]->id)) {
            ...
            $timestamp = Carbon::now();
            $content = "{$timestamp->format('Y:m:d H:i:s')}: Membership_card: All data synced already!!";
            Storage::append("error_sync_auto/error_membership_card_{$timestamp->format('Y_m_d')}.txt", $content); //Error
        }
    }
    ...
    public function Sync_Order_Auto() {
        ...
        if (count($syncOrderAuto) > 0) {
            foreach ($syncOrderAuto as $key => $value) {
                ...
                $myJSON = json_encode($myObj3, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
                $id_user_data = $this->syncAutoDB411Service->getIdUserByDataSync($value->MaKH);
                $branch_data_id = $this->syncAutoDB411Service->getIdBranchByDataSync($value->MaCH);
                $branch_id = isset($branch_data_id[0]->id) ? $branch_data_id[0]->id : null;
                ...
                if ($value->NhanVienSuaChua != null) {
                    $id_staff_data = $this->syncAutoDB411Service->getIdStaffByDataSync($value->NhanVienSuaChua);
                } else {
                    $id_staff_data = $this->syncAutoDB411Service->getIdStaffByDataSync($value->MaNV);
                }
                $id_staff = isset($id_staff_data[0]->staff_id) ? $id_staff_data[0]->staff_id : null;
                if (isset($id_user_data[0]->id)) {
                    $id_user = $id_user_data[0]->id;
                    $vehicle_number_data = $this->syncAutoDB411Service->getVehicleNumber($id_user);
                    ...
                    Storage::append("error_sync_auto/error_order_{$timestamp->format('Y_m_d')}.txt", $content); //Error
                }

                
                if ($value->SYS_CHANGE_OPERATION == "I") {
                    $checkExist_SoCT = $this->syncAutoDB411Service->checkExist_SoCT($value->SoCT);
                    if(isset($checkExist_SoCT[0]->id)) {
                        ...
                        if ($value->MaPSC != null) {
                            $dataDetailRepairVehicles = $this->syncAutoDB411Service->getDetailRepairVehicle($value->MaPSC);
                            if (count($dataDetailRepairVehicles) > 0) { 
                                foreach ($dataDetailRepairVehicles as $index => $detailRepairVehicle) {
                                    ...
                                    $saveOrderDetail = $this->syncAutoDB411Service->saveOrderDetailRepairVehicle($order_id, $detailRepairVehicle->DienGiai, $created_at = null, $type = EOrderType::REPLACEABLE_ITEM, $detailRepairVehicle->DonGia, $detailRepairVehicle->SoLuong, $myJSONDetail);
                                }
                            }
                        }
                    }
